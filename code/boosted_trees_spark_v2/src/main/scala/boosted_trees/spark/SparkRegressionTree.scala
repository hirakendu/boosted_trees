...
object SparkRegressionTree {
	...
	def trainNode(node : Node, samples : RDD[Array[Double]], featureTypes : Array[Int],
...
//			collect.toList
	...
	def trainTree(samples: RDD[Array[Double]], featureTypes : Array[Int],
		...
		while (!nodesStack.isEmpty) {
			val (node, nodeSamples) : (Node,  RDD[Array[Double]]) = nodesStack.pop
	...
	def readTree(sc : SparkContext, nodesFile : String) : Node = {
		RegressionTree.readTree(sc.textFile(nodesFile).collect.toIterator)
	}
