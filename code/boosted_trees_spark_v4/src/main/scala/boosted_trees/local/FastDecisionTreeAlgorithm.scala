 ...
 */
class FastDecisionTreeAlgorithm[S <: LossStats[S]:Manifest](
      val loss: Loss[S],
  ...
  def calculateHistogramsByArrayAggregate(data: Array[LabeledPoint],
    ...
    val statsForNodesFeaturesBins: Array[Array[Array[S]]] =
        new Array(nodeIdsToSplit.max - nodeIdOffset + 1)
    for (nodeId <- nodeIdsToSplit) {
      val l: Int = nodeId - nodeIdOffset
      statsForNodesFeaturesBins(l) = new Array(numFeatures)
      for (j <- 0 to numFeatures - 1) {
        if (params.featureTypes(j) == 0) {
          statsForNodesFeaturesBins(l)(j) = new Array(params.quantilesForFeatures(j).length + 1)
        } else if (params.featureTypes(j) == 1) {
          statsForNodesFeaturesBins(l)(j) = new Array(params.cardinalitiesForFeatures(j))
        } else if (params.featureTypes(j) == -1) {
          statsForNodesFeaturesBins(l)(j) = new Array(1)
        }
        for (b <- 0 to statsForNodesFeaturesBins(l)(j).length - 1) {
          statsForNodesFeaturesBins(l)(j)(b) = Loss.zeroStats[S]()
        }
      }
    }
    ...
    filter(sampleWithNodeId => nodeIdsToSplit.contains(sampleWithNodeId._2)).
    foreach(sampleWithNodeId => {
      val sample: LabeledPoint = sampleWithNodeId._1
      val l: Int = sampleWithNodeId._2 - nodeIdOffset
      ...
      while (j < numFeatures) {
        ...
        if (params.featureTypes(j) == 0) {
          // Continuous feature.
          bj = DataEncoding.findQuantileBin(params.quantilesForFeatures(j), sample.features(j))
        } else if (params.featureTypes(j) == 1) {
          // Discrete feature.
          bj = features(j).toInt
        }
        statsForNodesFeaturesBins(l)(j)(bj).addSample(sample.label, sample.features(numFeatures - 1))
        j += 1
      }
    })
   ...
   */
  def trainLevel(level: Int, rootNode: Node, data: Array[LabeledPoint]): Unit = {
    ...
    while (numNodesToSplit > 0) {
      ...
      println("        Finding loss statistics histograms for Level # " + level +
          ": Batch # " + batch + ".")
      var initialTime: Long = System.currentTimeMillis

      var statsForBinsForFeaturesForNodes: Array[Array[Array[(Int, S)]]] =
