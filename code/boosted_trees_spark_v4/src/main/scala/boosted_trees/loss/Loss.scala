...
trait LossStats[S <: LossStats[S]] extends Serializable {
   ...
   */
  def addSample(y: Double, weight: Double = 1): S
...
abstract class Loss[S <: LossStats[S]:Manifest] extends Serializable {
   ...
   * May instead use static method Loss.zeroStats to reduce task sizes.
   */
  // def zeroStats()(implicit m: Manifest[S]): S = {
  //   m.erasure.getConstructor().newInstance().asInstanceOf[S]
  // }

  def lossStatsType(): ClassManifest[S] = {
    classManifest[S]
  }
  ...
  /**
   * Calculates the total weight of a set of samples using
   * their loss statistics. Default implementation assumes
   * equal weight of 1 and thus returns count.
   *
   * @param lossStats Loss statistics of a set of samples.
   */
  def weight(lossStats: S): Double = count(lossStats)
...
object Loss {
   ...
   * generic algorithms for arbitrary loss functions.
   */
  def zeroStats[S]()(implicit m: Manifest[S]): S = {
    m.erasure.getConstructor().newInstance().asInstanceOf[S]
  }
